...
if __name__ == '__main__':
    ...
    parser.add_argument('--save_folder', default='./image/', type=str, help='Dir to save txt results')
    parser.add_argument('--dataset_folder', default='./image/', type=str, help='dataset path')
    ...
    with torch.no_grad():
        ...
        test_dataset = glob.glob(testset_folder+'*')
        for image_path in tqdm(test_dataset):
            img0 = cv2.imread(image_path)  # BGR
            if img0 is None:
                continue
            boxes = detect(model, img0)
            img_name = re.split('/.',image_path)[-1]
            # --------------------------------------------------------------------
            save_name = opt.save_folder + img_name + ".txt"
